<template>
  <div>
    ...
    <!--需要将mdShow属性传递到Modal.vue中控制弹框的显示与隐藏   子组件modal.vue会触发父组件的close事件，自定义事件-->
...
export default {
  ...
  computed:{
    checkAllFlag(){
      //every方法当数组中所有对象都返回true时，整体才会返回true，才会勾全选
    ...
    totalPrice(){
      let money=0;
      //会监测cartList的变化，只要发生变化，就会重新计算总价
  ...
  methods:{
    //初始化购物车列表
    init(){
      //使用vueaxios可以将axios对象挂载到当前vue实例的对象上,不需要再导入axios
    ...
    //关闭时父组件会传递到子组件，以mdShow属性的方式，告诉子组件关掉
    closeModal(){
